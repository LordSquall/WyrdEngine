...
namespace Wyrd
{
	OpenGLRenderer::OpenGLRenderer()
	{
		...
		std::ifstream vertexStream("../../Wyrd/res/shaders/debug2D.vs");
		...
		std::ifstream fragmentStream("../../Wyrd/res/shaders/debug2D.fs");
		...
		if (!success)
		{
			...
			WYRD_ERROR("Debug Vertex Shader Compilation Failed: {0}", infoLog);
		}
		// fragment shader
		...
		if (!success)
		{
			...
			WYRD_ERROR("Debug Fragment Shader Compilation Failed: {0}", infoLog);
		}
		// link shaders
		...
		if (!success) {
			...
			WYRD_ERROR("Debug Shader Linking Failed: {0}", infoLog);
		}
		glDeleteShader(vertexShader);
		glDeleteShader(fragmentShader);
		WYRD_INFO("Debug Shader Compiled Succesfully");
