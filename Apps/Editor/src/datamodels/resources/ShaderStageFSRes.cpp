...
{
	ShaderStageFSRes::ShaderStageFSRes(const std::string& name, const UID& uid) : Resource(name, uid)
	{

	}
	
	Resource::IOResult ShaderStageFSRes::Load(const std::string& filepath)
	{
	...
	Resource::IOResult ShaderStageFSRes::Load(const jsonxx::Object& object)
	{
		/* name */
	...
	Resource::IOResult ShaderStageFSRes::Save(const std::string& filepath) {
	...
	const std::string ShaderStageFSRes::GetTypeTag()
	{
		return RESOURCE_TAG_SHADERSTAGE_FS;
	}
