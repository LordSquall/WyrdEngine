...
namespace OrisisEngine
{
	struct FrameworkConfig {
		...
		FrameworkConfig() : loaded(false), width(800), height(600), x(0), y(0), title()
		{
			memcpy(title, "Application", sizeof(title));
		}

		FrameworkConfig(int w, int h, int x, int y, string t) : loaded(true), width(w), height(h), x(x), y(y), title()
		{
